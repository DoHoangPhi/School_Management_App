...
const schema = z.object({
    ...
    fisrtName: z.string().min(2, { message: 'First name is required' }),
    lastName: z.string().min(2, { message: 'Last name is required' }),
    phone: z.string().min(10, { message: 'Phone number is required' }),
    address: z.string().min(5, { message: 'Address is required' }),
    bloodType: z.string().min(2, { message: 'Blood type is required' }),
    birthday: z.date().min(new Date('1900-01-01'), { message: 'Birthday is required' }),
...
const TeacherForm = ({ type, data }: { type: 'create' | 'update', data?: any }) => {
    ...
    return (
        <form className='flex flex-col gap-8' onSubmit={onsubmit()}>
            ...
            <span className='text-xs text-shadow-gray-400 font-medium'>Authentication Infomation</span>
            ...
            <span className='text-xs text-shadow-gray-400 font-medium'>Personal Infomation</span>
            <div className='flex justify-between flex-wrap gap-4'>
                <InputField label='First Name' name='firstName' defaultValue={data?.firstName} register={register} error={errors.fisrtName} />
